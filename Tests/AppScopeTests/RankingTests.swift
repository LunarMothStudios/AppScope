...
@Test func ranksUseSourceOrderAndNeverInventPosition() throws {
  ...
  let result = try Ranking.analyze(
    appID: "34", keyword: "budget", country: "us", results: results, limit: 200)
  ...
  let absent = try Ranking.analyze(
    appID: "99", keyword: "budget", country: "us", results: results, limit: 200)
...
@Test func malformedResultsCannotBecomeMissingRank() {
  #expect(throws: ScopeError.self) {
    try Ranking.analyze(
      appID: "34", keyword: "budget", country: "us", results: [["oops": true]], limit: 200)
  }
}
