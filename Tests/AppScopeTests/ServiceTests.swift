...
actor MockHTTP: HTTPTransport {
    ...
    init(_ responses: [(JSON, Int)]) { self.responses = responses.map { (try! $0.0.encoded(), $0.1, [:]) } }
    func send(_ request: URLRequest) async throws -> (Data, Int, [String: String]) {
        requests.append(request)
        guard !responses.isEmpty else { throw ScopeError("unexpected_request", "No more mock responses.") }
...
@Test func failedSearchPreservesHistoryAndCachedSuccessAvoidsNetwork() async throws {
    ...
    let transport = MockHTTP([(["error": "redacted"], 403), (["resultCount": 2, "results": [["trackId": 1, "trackName": "Rival", "userRatingCount": 1000], ["trackId": 12, "trackName": "Mine", "userRatingCount": 1]]], 200)])
    let scope = try AppScope(config: Configuration(directory: dir), http: HTTP(transport: transport, searchInterval: 0))
...
@Test func trackingIsIdempotentCountryScopedAndReportsMarkMissing() async throws {
    ...
    _ = try await scope.call("track_keywords", ["app_id": "12", "keywords": ["Budget", "budget", "saving"]])
    _ = try await scope.call("track_keywords", ["app_id": "12", "country": "gb", "keywords": ["budget"]])
    ...
    #expect(throws: ScopeError.self) { try ToolCatalog.validate("analyze_keyword", ["app_id": "12", "keyword": "a", "limit": 100000]) }
