...
public struct ScopeError: Error, LocalizedError, Sendable {
    ...
    public var json: JSON { .object(["status": "error", "code": .string(code), "message": .string(message)]) }
}
...
extension Value {
    ...
    public static func decode(_ data: Data) throws -> Value { try JSONDecoder().decode(Value.self, from: data) }
    ...
    public func jsonText(pretty: Bool = false) throws -> String { String(decoding: try encoded(pretty: pretty), as: UTF8.self) }
...
public enum Validate {
    public static func appID(_ value: String) throws -> String {
        guard value.range(of: "^[0-9]{1,20}$", options: .regularExpression) != nil else { throw ScopeError("invalid_app_id", "Use the numeric App Store app ID.") }
        return value
    }
    public static func country(_ value: String) throws -> String {
        ...
        guard code.range(of: "^[a-z]{2}$", options: .regularExpression) != nil,
              Locale.Region.isoRegions.contains(where: { $0.identifier.lowercased() == code }) else {
    ...
    public static func keyword(_ value: String) throws -> String {
        let text = value.precomposedStringWithCompatibilityMapping.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !text.isEmpty, text.count <= 100, text.rangeOfCharacter(from: .controlCharacters) == nil else {
            throw ScopeError("invalid_keyword", "A keyword must contain 1–100 characters without control characters.")
        }
        return text
    }
    public static func date(_ value: String) throws -> String {
        ...
        guard let date = formatter.date(from: value), formatter.string(from: date) == value else { throw ScopeError("invalid_date", "Use a valid YYYY-MM-DD date.") }
        return value
    }
}
